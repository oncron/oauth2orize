...
/**
 * Exchanges resource owner SecureID challenge credentials for access tokens.
 *
 * This exchange middleware is used to by clients to obtain an access token by
 * presenting the resource owner's SecureID challenge tokencredentials.  These credentials are
 ...
 * Examples:
 *
 *     server.exchange(oauth2orize.exchange.challenge(function(client, username, password, scope, done) {
...
module.exports = function(options, issue) {
  ...
  if (!issue) { throw new TypeError('oauth2orize.challenge exchange requires an issue callback'); }
  ...
  return function challenge(req, res, next) {
    ...
    try {
      ...
      if (arity == 6) {
        issue(client, username, passwd, scope, state, issued);
      } else if (arity == 5) {
